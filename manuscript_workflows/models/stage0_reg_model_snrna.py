#!/usr/bin/env python3
"""Stage-0 v11 variant using donor snRNA gene profiles as FiLM gene embeddings."""
...
class Stage0RegModelV11SnRNAGeneMLP(Stage0RegModelV11):
    """Replace Decima FiLM conditioning with donor-matched snRNA gene-profile embeddings.

    This keeps the v11 donor/log-mean baseline and residual-scale machinery intact.
    ...
    def forward(
        ...
        if self.mu_only:
            raise ValueError("Stage0RegModelV11SnRNAGeneMLP is intended for residual prediction, not mu_only")
...
class Stage0RegModelV11SnRNAGatedFusion(Stage0RegModelV11SnRNAGeneMLP):
    ...
    def forward(
        ...
        if self.mu_only:
            raise ValueError("Stage0RegModelV11SnRNAGatedFusion is intended for residual prediction, not mu_only")
...
class Stage0RegModelV11SnRNAScGPTGeneMLP(Stage0RegModelV11SnRNAGeneMLP):
...
class Stage0RegModelV11SnRNAScGPTGeneGatedFusion(Stage0RegModelV11SnRNAGatedFusion):
    """Use Decima embeddings with gated donor-specific scGPT gene-context prior."""
    ...
    _load_snrna_gene_profiles = Stage0RegModelV11SnRNAScGPTGeneMLP._load_snrna_gene_profiles
