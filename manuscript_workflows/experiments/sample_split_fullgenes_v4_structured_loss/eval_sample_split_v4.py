#!/usr/bin/env python3
"""Evaluate Stage-0 v4 checkpoint using sample split lists from config."""
...
class RunningStats:
    ...
    def update_summary(
        ...
        self.sum_pred += float(sum_pred)
        self.sum_true += float(sum_true)
        self.sum_pred2 += float(sum_pred2)
        self.sum_true2 += float(sum_true2)
        self.sum_pred_true += float(sum_pred_true)
        self.sum_sq_error += float(sum_sq_error)
...
def update_gene_stats(
    ...
    """Accumulate per-gene moments with one vectorized reduction per chunk."""
    pred = pred.detach().to(dtype=torch.float64)
    true = true.detach().to(dtype=torch.float64)
    mask = torch.isfinite(pred) & torch.isfinite(true)
    safe_pred = torch.where(mask, pred, torch.zeros_like(pred))
    safe_true = torch.where(mask, true, torch.zeros_like(true))
    diff = safe_pred - safe_true

    summaries = torch.stack(
        [
            mask.sum(dim=0).to(dtype=torch.float64),
            safe_pred.sum(dim=0),
            safe_true.sum(dim=0),
            (safe_pred * safe_pred).sum(dim=0),
            (safe_true * safe_true).sum(dim=0),
            (safe_pred * safe_true).sum(dim=0),
            (diff * diff).sum(dim=0),
        ],
        dim=1,
    ).cpu().numpy()

    for gid, values in zip(gene_ids, summaries):
        gene_stats.setdefault(gid, RunningStats()).update_summary(
            count=int(values[0]),
            sum_pred=float(values[1]),
            sum_true=float(values[2]),
            sum_pred2=float(values[3]),
            sum_true2=float(values[4]),
            sum_pred_true=float(values[5]),
            sum_sq_error=float(values[6]),
        )
    ...
    with torch.no_grad():
        for idx in range(len(ds)):
            ...
            for start, end in ranges:
                ...
                update_gene_stats(gene_stats, gene_ids, pred_cpu, true_cpu)

            print(f"[eval] completed sample {idx + 1}/{len(ds)}: {sample_name}", flush=True)
